  ...
  return (
    <div className="grid gap-4 xl:grid-cols-8 lg:grid-cols-6 md:grid-cols-4 sm:grid-cols-2">
      <Card className="bg-card py-2 gap-2">
        ...
        <CardContent className="p-3 pt-1">
          <div className={`text-lg font-bold ${isProfit ? "text-green-500" : "text-red-500"}`}>
            {hideValues
              ? "****"
              : `${isProfit ? "+" : "-"}$${Math.abs(allTimeProfit).toLocaleString("en-US", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}`}
          </div>
          <p className={`text-xs font-medium ${isProfit ? "text-green-500" : "text-red-500"}`}>
            {isProfit ? "up" : "down"} {Math.abs(allTimeProfitPercentage).toFixed(2)}%
          </p>
        </CardContent>
      </Card>
      ...
      <Card className="bg-card py-2 gap-2">
        ...
        <CardContent className="p-3 pt-1">
          <div className="text-lg font-bold text-foreground">{totalInvestmentMethods}</div>
      ...
      <Card className="bg-card py-2 gap-2">
        ...
        <CardContent className="p-3 pt-1">
          <div className="text-lg font-bold text-foreground">{activeTransactions}</div>
